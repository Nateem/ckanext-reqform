...
def validate_reqform(context, request_data):
    ...
    if 'Title' not in errors and not avoid_existing_title_check:
        if db.DataRequest.reqform_exists(request_data['title']):
...
def validate_reqform_closing(context, request_data):
...
def validate_comment(context, request_data):
    ...
    try:
        reqform = tk.get_action(constants.SHOW_REQFORM)(context, {'id': request_data['reqform_id']})
    except Exception:
        raise tk.ValidationError({tk._('Request Form'): [tk._('Request Form not found')]})
