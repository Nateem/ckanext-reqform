...
def init_db(model):
    ...
    if DataRequest is None:
        ...
        class _DataRequest(model.DomainObject):
            ...
            @classmethod
            def reqform_exists(cls, title):
                '''Returns true if there is a Request Form with the same title (case insensitive)'''
                query = model.Session.query(cls).autoflush(False)
            ...
            @classmethod
            def get_open_reqforms_number(cls):
        ...
        datarequests_table = sa.Table('reqform', model.meta.metadata,
    ...
    if Comment is None:
        class _Comment(model.DomainObject):
            ...
            @classmethod
            def get_ordered_by_date(cls, reqform_id, desc=False):
                '''Personalized query'''
                query = model.Session.query(cls).autoflush(False)
                order_by_filter = cls.time.desc() if desc else cls.time.asc()
                return query.filter_by(reqform_id=reqform_id).order_by(order_by_filter).all()

            @classmethod
            def get_comment_reqforms_number(cls, **kw):
                '''
        ...
        comments_table = sa.Table('datarequests_comments', model.meta.metadata,
            sa.Column('id', sa.types.UnicodeText, primary_key=True, default=uuid4),
            sa.Column('user_id', sa.types.UnicodeText, primary_key=False, default=u''),
            sa.Column('reqform_id', sa.types.UnicodeText, primary_key=True, default=uuid4),
    ...
    if DataRequestFollower is None:
        class _DataRequestFollower(model.DomainObject):
            ...
            @classmethod
            def get_reqform_followers_number(cls, **kw):
                '''
