...
def dictice_ddbb_response(reqform):
    return {
        'id': reqform.id,
        ...
        'close_time': str(reqform.close_time) if reqform.close_time else reqform.close_time,
...
def _generate_basic_ddbb_response(number, organizations=None, closed=None):
    ...
    for n in range(number):
        response.append(_generate_basic_reqform(organization_id=organizations[n], 
                                                    closed=closed[n]))

    return response


def _generate_basic_reqform(id=DATAREQUEST_ID, user_id='example_uuidv4_user',
                                title='This is a title', description='This is a basic description',
                                organization_id='example_uuidv4_organization', closed=False):
    reqform = MagicMock()
    reqform.id = id
    reqform.user_id = user_id
    reqform.title = title
    reqform.description = description
    reqform.organization_id = organization_id
    reqform.open_time = datetime.datetime.now()
    reqform.closed = closed
    reqform.close_time = None
    reqform.accepted_dataset_id = None
    reqform.accepted_dataset = {'test': 'test1', 'test2': 'test3'}

    return reqform


def _generate_basic_comment(id=COMMENT_ID, user_id='example_uuidv4_user',
                            comment='Example Comment', reqform_id='example_dr_id'):
    comment = MagicMock()
    comment.id = id
    comment.user_id = user_id
    comment.comment = comment
    comment.reqform_id = reqform_id
...
ddbb_response_1 = [_generate_basic_reqform(organization_id=None)]
...
ddbb_response_2 = [
    _generate_basic_reqform(organization_id=org1, closed=True),
    _generate_basic_reqform(organization_id=org2, closed=False),
    _generate_basic_reqform(organization_id=org1, closed=True),
    _generate_basic_reqform(organization_id=org3, closed=False),
    _generate_basic_reqform(organization_id=org2, closed=True),
    _generate_basic_reqform(organization_id=org1, closed=False),
    _generate_basic_reqform(organization_id=None, closed=True),
    _generate_basic_reqform(organization_id=None, closed=True),
