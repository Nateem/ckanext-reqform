...
import ckanext.reqform.validator as validator
import unittest
import random
...
class ValidatorTest(unittest.TestCase):
    ...
    def setUp(self):
        ...
        validator.db.DataRequest.reqform_exists.return_value = False
    ...
    def test_validate_valid_data_request(self, avoid_existing_title_check):
        ...
        self.assertIsNone(validator.validate_reqform(context, self.request_data))
        ...
        if avoid_existing_title_check:
            self.assertEquals(0, validator.db.DataRequest.reqform_exists.call_count)
        else:
            validator.db.DataRequest.reqform_exists.assert_called_once_with(self.request_data['title'])
    ...
    def test_validate_name_description(self, field, value, title_exists, excepction_msg):
        ...
        validator.db.DataRequest.reqform_exists.return_value = title_exists

        with self.assertRaises(self._tk.ValidationError) as c:
            validator.validate_reqform(context, self.request_data)
    ...
    def test_close_invalid_accepted_dataset(self):
        ...
        with self.assertRaises(self._tk.ValidationError) as c:
            validator.validate_reqform_closing(context, {'id': 'dr_id', 'accepted_dataset_id': accepted_ds_id})
    ...
    def test_comment_invalid(self, request_data, field, message):
        context = {}
        request_data['reqform_id'] = 'exmaple'
    ...
    def test_comment_invalid_reqform(self):
        show_reqform = validator.tk.get_action.return_value
        show_reqform.side_effect = self._tk.ObjectNotFound('Store Not found')

        self.test_comment_invalid({'reqform_id': 'non_existing_dr'}, 'Request Form',
    ...
    def test_comment_valid(self):
        ...
        self.assertEquals(result, show_reqform.return_value)
